...
public class GuiFancySlotServer extends GuiFancySlot {
	/** Instance to the GUI this list is on. */
	final GuiFancyMultiplayer parentGui;
	final Integer slotHeight;

	public GuiFancySlotServer(GuiFancyMultiplayer par1GuiMultiplayer)
	{
		...
		this.slotHeight = super.slotHeight;
	}
	...
	{
		return GuiFancyMultiplayer.getInternetServerList(this.parentGui).countServers() + GuiFancyMultiplayer.getListOfLanServers(this.parentGui).size() + 1;
	}
	...
	{
		if (par1 < GuiFancyMultiplayer.getInternetServerList(this.parentGui).countServers() + GuiFancyMultiplayer.getListOfLanServers(this.parentGui).size())
		{
			...
			GuiFancyMultiplayer.getButtonDelete(this.parentGui).enabled = var6;

			if (par2 && var5)
			{
				GuiFancyMultiplayer.func_74008_b(this.parentGui, par1);
			}
			else if (var6 && GuiScreen.isShiftKeyDown() && var3 >= 0 && var3 < GuiFancyMultiplayer.getInternetServerList(this.parentGui).countServers())
			{
				GuiFancyMultiplayer.getInternetServerList(this.parentGui).swapServers(var3, GuiFancyMultiplayer.getSelectedServer(this.parentGui));
			}
		}
	}
	...
	{
		return par1 == GuiFancyMultiplayer.getSelectedServer(this.parentGui);
	}
	...
	{
		if (par1 < GuiFancyMultiplayer.getInternetServerList(this.parentGui).countServers())
		{
			this.func_77247_d(par1, par2, par3, par4, par5Tessellator);
		}
		else if (par1 < GuiFancyMultiplayer.getInternetServerList(this.parentGui).countServers() + GuiFancyMultiplayer.getListOfLanServers(this.parentGui).size())
		{
	...
	{
		LanServer var6 = (LanServer)GuiFancyMultiplayer.getListOfLanServers(this.parentGui).get(par1 - GuiFancyMultiplayer.getInternetServerList(this.parentGui).countServers());
	...
	private void func_77249_c(int par1, int par2, int par3, int par4, Tessellator par5Tessellator)
	{
		...
		switch (GuiFancyMultiplayer.getTicksOpened(this.parentGui) / 3 % 4)
		{
			case 0:
			default:
				var6 = "O o o";
	...
	{
		ServerData var6 = GuiFancyMultiplayer.getInternetServerList(this.parentGui).getServerData(par1);

		synchronized (GuiFancyMultiplayer.getLock())
		{
			if (GuiFancyMultiplayer.getThreadsPending() < 5 && !var6.field_78841_f)
			{
				var6.field_78841_f = true;
				var6.pingToServer = -2L;
				var6.serverMOTD = "";
				var6.populationInfo = "";
				GuiFancyMultiplayer.increaseThreadsPending();
		...
		{
			GuiFancyMultiplayer.getAndSetLagTooltip(this.parentGui, var12);
		}
	}
}
