...
public class GuiFancyMainMenu extends GuiFancyScreen {
	...
	public void initGui() {
		...
		if (ForgeInstalled) {
			buttonList.add(new GuiFancyButton(8, width, height - 55, "Mods", 2));
		}

		buttonList.add(new GuiFancyButton(100, 0, height - 55, "Friends", 1));

		this.buttonList.add(new GuiFancyButton(0, width, height - 75, var2.translateKey("menu.options"), 2));
		this.buttonList.add(new GuiFancyButton(4, width, height - 30, var2.translateKey("menu.quit"), 2));

		this.buttonList.add(new GuiFancyButton(5, width, height - 95, var2.translateKey("options.language").replace("...", ""), 2));
		this.field_92025_p = "";
	...
	private void func_96137_a(StringTranslate par1StringTranslate, int par2, int par3)
	{
		if (this.field_96141_q)
		{
			...
			{
				field_96140_r = true;
				(new ThreadFancyTitleScreen(this, par1StringTranslate, par2, par3)).start();
			}
	...
	{
		this.buttonList.add(new GuiFancyButton(3, width, height - 115, par1StringTranslate.translateKey("menu.online"), 2));
	}
	...
	private void addSingleplayerMultiplayerButtons(int par1, int par2,
		...
		buttonList.add(new GuiFancyButton(1, 0, height - 120, par3StringTranslate.translateKey("menu.singleplayer"), 1));
		buttonList.add(new GuiFancyButton(2, 0, height - 80, par3StringTranslate.translateKey("menu.multiplayer"), 1));
		buttonList.add(new GuiFancyButton(6, 0, height - 140, "Continue", 1));
		buttonList.add(new GuiFancyButton(9, 0, height - 100, "Connect", 1));
	}
	...
	private void addDemoButtons(int par1, int par2,
			StringTranslate par3StringTranslate) {
		this.buttonList.add(new GuiFancyButton(11, 0, this.height - 120, par3StringTranslate.translateKey("menu.playdemo"), 1));
		this.buttonList.add(buttonResetDemo = new GuiFancyButton(12, 0, height - 90, par3StringTranslate.translateKey("menu.resetdemo"), 1));
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		...
		if (par1GuiButton.id == 9) {
			if (mc.gameSettings.lastServer != "") {
				try {
					mc.displayGuiScreen(new GuiConnecting(null, this.mc, new ServerData("Last Server", URLDecoder.decode(mc.gameSettings.lastServer, "UTF-8"))));
				} catch (Exception e) {
				}
			}
			return;
	...
	static Minecraft func_98058_a(GuiFancyMainMenu par0GuiMainMenu)
	{
		return par0GuiMainMenu.mc;
	}

	static void func_98061_a(GuiFancyMainMenu par0GuiMainMenu, StringTranslate par1StringTranslate, int par2, int par3)
	{
