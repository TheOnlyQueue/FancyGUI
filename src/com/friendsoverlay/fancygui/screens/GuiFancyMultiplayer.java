...
public class GuiFancyMultiplayer extends GuiFancyScreen {
	...
	private GuiFancySlotServer serverSlotContainer;
	private ServerList internetServerList;

	/** Index of the currently selected server */
	private int selectedServer = -1;
	private GuiFancyButton field_96289_p;

	/** The 'Join Server' button */
	private GuiFancyButton buttonSelect;

	/** The 'Delete' button */
	private GuiFancyButton buttonDelete;
	...
	public GuiFancyMultiplayer(Minecraft mc, GuiScreen oldScreen)
	{
		super(mc, oldScreen);
		this.mc = super.mc;
	...
	public void initGui()
	{
		...
		if (!this.field_74024_A)
		{
			...
			this.serverSlotContainer = new GuiFancySlotServer(this);
		}
		else
		{
	...
	public void initGuiControls()
	{
		...
		this.buttonList.add(this.field_96289_p = new GuiFancyButton(7, 0, this.height - 28, 100, 20, var1.translateKey("selectServer.edit"), 1));
		this.buttonList.add(this.buttonDelete = new GuiFancyButton(2, this.width / 2 - 60, this.height - 28, 100, 20, var1.translateKey("selectServer.delete"), 3));
		this.buttonList.add(this.buttonSelect = new GuiFancyButton(1, 0, this.height - 52, 100, 20, var1.translateKey("selectServer.select"), 1));
		this.buttonList.add(new GuiFancyButton(4, this.width / 2, this.height - 52, 100, 20, var1.translateKey("selectServer.direct"), 3));
		this.buttonList.add(new GuiFancyButton(3, this.width, this.height - 52, 100, 20, var1.translateKey("selectServer.add"), 2));
		this.buttonList.add(new GuiFancyButton(8, this.width / 2 + 60, this.height - 28, 100, 20, var1.translateKey("selectServer.refresh"), 3));
		this.buttonList.add(new GuiFancyButton(0, this.width, this.height - 28, 100, 20, var1.translateKey("gui.cancel"), 2));
	...
	protected void actionPerformed(GuiButton par1GuiButton)
	{
		if (par1GuiButton.enabled)
		{
			...
			else if (par1GuiButton.id == 4)
			{
				...
				this.mc.displayGuiScreen(new GuiFancyScreenServerList(this, this.theServerData = new ServerData(StatCollector.translateToLocal("selectServer.defaultName"), "")));
			}
			else if (par1GuiButton.id == 3)
			{
				this.addClicked = true;
				this.mc.displayGuiScreen(new GuiFancyScreenAddServer(this, this.theServerData = new ServerData(StatCollector.translateToLocal("selectServer.defaultName"), "")));
			}
			else if (par1GuiButton.id == 7)
			{
				...
				this.mc.displayGuiScreen(new GuiFancyScreenAddServer(this, this.theServerData));
			}
	...
	static ServerList getInternetServerList(GuiFancyMultiplayer par0GuiMultiplayer)
	{
		return par0GuiMultiplayer.internetServerList;
	}

	static List getListOfLanServers(GuiFancyMultiplayer par0GuiMultiplayer)
	{
		return par0GuiMultiplayer.listofLanServers;
	}

	static int getSelectedServer(GuiFancyMultiplayer par0GuiMultiplayer)
	{
		return par0GuiMultiplayer.selectedServer;
	}

	static int getAndSetSelectedServer(GuiFancyMultiplayer par0GuiMultiplayer, int par1)
	{
	 ...
	 */
	static GuiButton getButtonSelect(GuiFancyMultiplayer par0GuiMultiplayer)
	{
	 ...
	 */
	static GuiButton getButtonEdit(GuiFancyMultiplayer par0GuiMultiplayer)
	{
	 ...
	 */
	static GuiButton getButtonDelete(GuiFancyMultiplayer par0GuiMultiplayer)
	{
		return par0GuiMultiplayer.buttonDelete;
	}

	static void func_74008_b(GuiFancyMultiplayer par0GuiMultiplayer, int par1)
	{
		par0GuiMultiplayer.joinServer(par1);
	}

	static int getTicksOpened(GuiFancyMultiplayer par0GuiMultiplayer)
	{
	...
	static String getAndSetLagTooltip(GuiFancyMultiplayer par0GuiMultiplayer, String par1Str)
	{
